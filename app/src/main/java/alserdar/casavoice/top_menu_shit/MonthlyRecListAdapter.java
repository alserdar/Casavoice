...
public class MonthlyRecListAdapter extends BaseAdapter implements Adapter {

    private Context c  ;
    private List<UserModel> list ;
    private int counter = 0;

    public MonthlyRecListAdapter(Context context , List<UserModel> model ) {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflater.inflate(R.layout.monthly_rec_list, parent, false);
        }

        final UserModel model = (UserModel) getItem(position);
        final ImageView picRecMonthly = view.findViewById(R.id.imageForRecMonthly);
        final TextView theSender = view.findViewById(R.id.userNameForRecMonthly);
        TextView howMuch = view.findViewById(R.id.muchSendForRecMonthly);

        theSender.setText(model.getUserName());
        howMuch.setText(String.valueOf(model.getMonthlyRecBalance()));

        final String originalName = model.getOriginalName();
        final String originalUID = model.getTheUID() ;
        LoadingPics.loadPicsForTopMenu(c.getApplicationContext() , originalUID , picRecMonthly);
