...
public class DailySenderListAdapter extends BaseAdapter {

    private Context c  ;
    private List<UserModel> list ;


    public DailySenderListAdapter(Context context , List<UserModel> model ) {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflater.inflate(R.layout.daily_sender_list, parent, false);
        }

        final UserModel model = (UserModel) getItem(position);

        final ImageView picSenderDaily = view.findViewById(R.id.imageForSenderDaily);
        final String originalName = model.getOriginalName();
        final  String originalUID = model.getTheUID() ;
        LoadingPics.loadPicsForTopMenu(c.getApplicationContext() , originalUID , picSenderDaily);


        final TextView theSender = view.findViewById(R.id.userNameForSenderDaily);
        TextView howMuch = view.findViewById(R.id.muchSendForSenderDaily);

        theSender.setText(model.getUserName());
        howMuch.setText(String.valueOf(model.getDailySendBalance()));
