...
public class MonthlySenderListAdapter extends BaseAdapter implements Adapter {

    private Context c  ;
    private List<UserModel> list ;

    public MonthlySenderListAdapter(Context context , List<UserModel> model ) {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflater.inflate(R.layout.monthly_sender_list, parent, false);
        }

        final UserModel model = (UserModel) getItem(position);
        final ImageView picSenderMonthly = view.findViewById(R.id.imageForSenderMonthly);
        final TextView theSender = view.findViewById(R.id.userNameForSenderMonthly);
        TextView howMuch = view.findViewById(R.id.muchSendForSenderMonthly);

        theSender.setText(model.getUserName());
        howMuch.setText(String.valueOf(model.getMonthlySendBalance()));
        final String originalName = model.getOriginalName();
        final String originalUID = model.getTheUID() ;
        LoadingPics.loadPicsForTopMenu(c.getApplicationContext() , originalUID , picSenderMonthly);
