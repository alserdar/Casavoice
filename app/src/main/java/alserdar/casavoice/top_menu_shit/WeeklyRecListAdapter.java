...
public class WeeklyRecListAdapter extends BaseAdapter implements Adapter {

    private Context c  ;
    private List<UserModel> list ;

    public WeeklyRecListAdapter(Context context , List<UserModel> model ) {
    ...
    public View getView(final int position, View view, ViewGroup parent) {
        ...
        if (view == null) {
            ...
            view = inflater.inflate(R.layout.weekly_rec_list, parent, false);
        }

        final UserModel model = (UserModel) getItem(position);
        final ImageView picRecWeekly = view.findViewById(R.id.imageForRecWeekly);
        final TextView theSender = view.findViewById(R.id.userNameForRecWeekly);
        TextView howMuch = view.findViewById(R.id.muchSendForRecWeekly);

        theSender.setText(model.getUserName());
        howMuch.setText(String.valueOf(model.getWeeklyRecBalance()));
        final String originalName = model.getOriginalName();
        final String originalUID = model.getTheUID() ;
        LoadingPics.loadPicsForTopMenu(c.getApplicationContext() , originalUID , picRecWeekly);
